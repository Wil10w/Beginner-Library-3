...
#Add your code here!
def next_fib(alist, AnInt):
    #an empty variable to count the loops
    count = 0
    #while count is not equal to the number of times we need to a fib to list
    while count != AnInt:
        #add to count on every iteration
        count += 1
        #add the last two indexes together
        adding = alist[-1] + alist[-2]
