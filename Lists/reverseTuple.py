...
def unpack_and_reverse(tuple):
    new_tup = () #  making a new, emoty tuple
    for i in reversed(tuple): # using reversed() to flip the tuple(tuple)
        new_tup = new_tup + (i,) # adding the info to the tuple
    return new_tup[-3:] # returning only the last 3 variables in the tuple
