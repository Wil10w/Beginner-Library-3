...
def name_fixer(list_of_Names):
    ...
    for i in range(len(list_of_Names)):# a quick loop to get an index number for each of the strings in the list
        ...
        if ',' in list_of_Names[i]:# if there is a comma in the string
            names = list_of_Names[i].split(',')# split the string into a tuple

            names = names[1].strip() + ' ' + names[0]# concat it and get rid of any unwanted whitespace with strip
            names.strip() # I don't know why this is here :)
            newNamesList.append(names)# Append the string into the new list

        else:
            newNamesList.append(list_of_Names[i])# if the string doesn't have a comma, append as it is.
