...
#Add your code here!
def inside_search(alist, search_word):
    list = []# make an emplty list to hold the index locations of search_word
    for i in range(len(alist)):# loop through the range/index (0,1,2)

        if search_word in alist[i]: # use the result of the loop(i) to see if search_word is in (i) postion in list
            list.append(i)# if it is, add the index number to the new list
    return list# once the loop is finished, return either and empty list or the result of the loop
