...
def average_heart_rate(list):
    allBeats = 0# a variable to add all the +100 integers
    count = 0# to count only the +100 ints

    for i in list:# looping through the list of ints
        if i >= 100:# if the int is over or = to 100
            allBeats += i # adds the int to create a whole number not a list
            count +=1# plus 1 to get the number to // by for the average

    return allBeats // count# returning the avergae using floor division (//)
