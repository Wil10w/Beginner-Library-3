...
def attendance_check(roster, present):
    absent = []#    making an empty list

    for r in roster:# looping through the first string

        if not r in present: # checking to see if r is NOT in the second list
            absent.append(r)# if it's not present, adding it to the new list
            absent.sort()#  aphabetizing the new lsit 
    return absent
