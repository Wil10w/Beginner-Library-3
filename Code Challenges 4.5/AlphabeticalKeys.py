...
def alphabetical_keys(adict):
    #make the dictionary into a list with the code below
    alist = list(adict.items())
    #then we sort the list
    alist.sort()
    #a variable for the string we will concat
    astring = ''
    #looping over the list's length of the list.
    #this allows for longer lists in the future because next time it may be 15 names long
    for i in range(0,len(alist)):
        #this list is a list of tuples. we loop over each tuple (i)
        #the variable holds index 0 and 1 of tuple[i] with a newline at the end.
        #this is done for each loop.
       astring += alist[i][0] +': ' + str(alist[i][1]) + '\n'
    # we return the finished variable(astring)
    return astring
