...
#Add your code here!
#TL;DR = times all ints together -  up to -1. then return as
# timesNums ** (1/howManyInts)
def geometric_rainfall(alsit):
    #two empty variable, one to collect all the nums and another to count the
    #number of items in the list.
    count = 0
    allnums = 1
    #loop over list
    for i in alsit:
        #if i is -1 the loop should stop
        if i == -1:
            break
        #else add 1 to count and times i to allnums
        else:
            count += 1
            allnums *= i
    #return this formula
    return round(allnums ** (1/count), 1)
