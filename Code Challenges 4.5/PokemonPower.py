...
def total_stats(listOfPokemon):
    # an empty dictioanry
    allThePokemon = {}
    #loopin over the list of pokemons and their stats
    for i in listOfPokemon:
        #I created a variable to hold the name of the pokemon becuase when I did i['name'] for the
        #dictionary it added 'name' and not the pokemon's name - so I just did the same code into
        #a var instead and added the variable as the key.
        name = i['name']
        #added all the attributes together as is asked in the instructions (line 14)
        #I just used the Keys for each stat as they we never changing in this challenge, rather
        # than using aother loop or something.
        math = i['hp'] + i['attack'] + i['defense'] + i['special attack'] + i['special defense'] + i['speed']
        #added the variable - name as the Key and variable - math as the value
        allThePokemon[name] = math
    return allThePokemon
