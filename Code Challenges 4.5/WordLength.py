...
def word_lengths(astring):
    #first we strip down the sentence by removing all punctualtion and lowercasing everything.
    astring = astring.lower()
    astring = astring.strip('.')
    #there are extra strips and a replace to remove the punctuation that kept turning up in the
    #autograder in my course.
    astring = astring.strip('?')
    astring = astring.replace(',', '')
    astring = astring.strip('!')
    #creating a list from the string so we can iterate over it.
    alist = astring.split()
    #an empty variable so we can count the letters
    count = 0
    #an emply dict to return
    adict = {}
    #looping over the list of dictionaries we are given
    for i in alist:
        #resetting the count to 0
        count = 0
        #collectiong i into a permenant varible(word) because I keep getting an error
        #when I call it in the nested loop below... the only thing I can think is that
        # 'i' is not carried over into the loop and j becomes the only iterated var.
        word = i
        #looping through i so we can count every letter
        for j in i:
            #plus one for every iteration
            count +=1
        #collecting everything from all the j iterations[count] with i[word] as its Key, we add [count] to the
        #dictioanry
        adict[word] = count
