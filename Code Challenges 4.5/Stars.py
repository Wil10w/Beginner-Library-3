...
#TL;DR add movies/TV programs into a list under stars names
#(if they were in the program/movies) sort alphabetically
def stars(movie, tv):
    #empty dictionary to return
    adict = {}
    #looping over movies first
    for dicts in movie.items():
        #unpacking Key and Value
        film, names = dicts
        #looping over the list[names]
        for i in names:
            #we try to append, but if there isn't a list(first time maybe)
            #we catch the KeyError and make the list and Key...
            try:
                adict[i].append(film)
            except KeyError:
                #here
                adict[i] = [film]
    #then we loop through the other dict TV
    for dicts in tv.items():
        #unpack the same way as the one above
        film, names = dicts
        #again we iterate through the list of names
        for i in names:
            #we try to append the Key and list
            try:
                adict[i].append(film)
            except KeyError:
                #if we have new actors we add them here
                adict[i] = [film]
    # we are sorting here
    for list in adict.items():
        #loop through the dictionary and
        #instead of unpacking the tuple(.item) we
        #just use [1] to call the list and then take
        #the while list and sort it.
        list[1].sort()
    #we return the dictionary of lists.
    return adict
