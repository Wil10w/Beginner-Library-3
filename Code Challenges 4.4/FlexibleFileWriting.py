...
def write_weird_file(afile, alist, mode='w', sort_first=False, reverse_first=False ):

    #   first we start by checking all the default parameters.
    if sort_first:# is sort is True
        alist.sort()# sort list
    if reverse_first:# is Reverse is True
        alist.reverse()# reverse the list
    # how we deal with the two modes... this can also be done with output_file = open(afile, mode).
    # This will remove the if statements... maybe I'll do this one day.
    if mode == 'w':# if mode is write
        output_file = open(afile, 'w')
        for i in alist:# we then loop through the list(alist)
            output_file.write(str(i) + '\n')# and write line by line to file
        output_file.close()# close the god damn file
    elif mode == 'a':#if mode is append
        output_file = open(afile, 'a')# open file
        for i in alist:# loop through list
            output_file.write(str(i) + '\n')# write to en of file
