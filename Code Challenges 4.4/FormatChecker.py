...
def format_checker(afile):
    #opening file
    outputfile = open(afile, 'r')
    #creating a variable to add up weights line[4]
    afloat = 0
    #loop through the line in the file
    for aline in outputfile:
        #create a list from the line of data
        alist = aline.split()


        #use a try/except to check that each line has the criteria listed above in the instructions
        try:
            #can [0] be changed into an int?
            int(alist[0])
            #can [2] be changed into an int?
            int(alist[2])
            #can [3] be changed to an int?
            int(alist[3])
            #can [4] be changed into a float?
            float(alist[4])
            #if this is all correct, the program will move onto the next 'try'
        ...
        #a sencond 'try' because this correct result will cause a False.
        try:
            #it this an int?
            int(alist[1])
            #if yes, it's the wrong input and will return a False.
            return False
        except:
            #if not an int, GODD correct input, continue with program
            pass
        #add 5th word/num to this variable as a final sum will be needed for a True/False outcome
        afloat += float(alist[4])
    #after loop has finished, if all added floats are not equal to 1
    if not afloat == 1:
        return False

    #if everything is True(or not False)
    outputfile.close()
    return True
...
print(format_checker("sample_1.txt"))
#print(format_checker("sample_2.txt"))
